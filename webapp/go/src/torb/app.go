...
func getEvents(ctx context.Context, all bool) ([]*Event, error) {
	...
	for i, v := range events {
		event, err := getEvent(ctx, *v, -1, true)
		if err != nil {
			return nil, err
		}
...
func getEvent(ctx context.Context, event Event, loginUserID int64, cache bool) (*Event, error) {
	...
	rows, err := db.QueryContext(ctx, "SELECT * FROM reservations WHERE event_id = ? AND canceled_at IS NULL GROUP BY event_id, sheet_id HAVING reserved_at = MIN(reserved_at)", ev.ID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()

	ress := map[int64]Reservation{}
	for rows.Next() {
...
func main() {
	...
	e.GET("/api/users/:id", func(c echo.Context) error {
		...
		rows, err := db.QueryContext(ctx, "select r.*, s.rank AS sheet_rank, s.num AS sheet_num, s.price AS sheet_price FROM reservations r INNER JOIN sheets s ON s.id = r.sheet_id WHERE r.user_id = ? ORDER BY IFNULL(r.canceled_at, r.reserved_at) DESC LIMIT 5", user.ID)
		if err != nil {
			return err
		}
		defer rows.Close()
		...
		for rows.Next() {
			...
			event, err := getEvent(ctx, rawEvent, -1, true)
			if err != nil {
				return err
			}
			for k := range event.Sheets {
	...
	e.GET("/api/events/:id", func(c echo.Context) error {
		...
		event, err := getEvent(ctx, rawEvent, loginUserID, true)
		if err != nil {
			if err == sql.ErrNoRows {
				return resError(c, "not_found", 404)
			}
	...
	e.POST("/api/events/:id/actions/reserve", func(c echo.Context) error {
		...
		event, err := getEvent(ctx, rawEvent, user.ID, false)
		if !event.PublicFg {
			return resError(c, "invalid_event", 404)
		}
	...
	e.POST("/admin/api/events", func(c echo.Context) error {
		...
		event, err := getEvent(ctx, rawEvent, -1, false)
		if err != nil {
			return err
		}
	...
	e.POST("/admin/api/events/:id/actions/edit", func(c echo.Context) error {
		...
		e, err := getEvent(ctx, rawEvent, -1, false)
		if err != nil {
			return err
		}
		c.JSON(200, e)
